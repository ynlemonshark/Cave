...
class Rock(Drawable):
    def __init__(self, pos, size):
        ...
        self.image = pygame.image.load("dd.png")
...
class Ship(Drawable):
    def __init__(self):
        ...
        self.accel = 0
        self.explode = False
        self.image = pygame.image.load("man2.png")
...
def key_event_handler(keymap, ship):
    ...
    if K_LEFT in keymap:
        ...
        ship.accel = min(5, ship.accel + .2)
    elif K_DOWN in keymap:
        ship.accel = max(-5, ship.accel - .1)
...
def main():
    ...
    while True:
        ...
        if not game_over:
            ...
            for rock in rocks:
                rock.tick()
                if 0:#rock.rect.colliderect(ship.rect):
                    ship.explode = True
                    game_over = True
            fire = False
            for shot in shots:
